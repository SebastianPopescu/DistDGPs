...
import numpy as np
DTYPE=tf.float32
from DDGP.conditional_GP import *
...
class propagate_layers(model_details):
    ...
    def propagate_layer_euclidean(self, X, X_mean_function, l, training_time, full_cov):
        ...
        output_now = conditional_GP_Euclidean(Xnew = X, X = Z, Xnew_mean_function = X_mean_function, 
    ...
    def propagate_layer_wasserstein(self, X_mean, X_var, X_mean_function, l, training_time):
        ...
        with tf.compat.v1.variable_scope('num_layer_'+str(l), reuse=tf.compat.v1.AUTO_REUSE):
            ...
            Z_mean = tf.compat.v1.get_variable(initializer =  tf.random_uniform_initializer(minval=-0.5,
                maxval=0.5), shape = (self.num_inducing[l-1], self.dim_layers[l-1]),
                dtype=DTYPE,name='Z_mean')

            ### variance of inducing points location is taken to be diagonal multivariate #####

            matrice_dummy = tf.ones(shape = [self.num_inducing[l-1], self.dim_layers[l-1]], dtype=DTYPE) * -5.0
            Z_sqrt_var = tf.compat.v1.get_variable(initializer = matrice_dummy,
                dtype=DTYPE,name='Z_var_sqrt')			
            #Z_var = tf.square(Z_sqrt_var)
            Z_var = tf.math.softplus(Z_sqrt_var)
        ...
        output_now = conditional_GP_Wasserstein(Xnew_mean = X_mean, Xnew_var = X_var, X_mean = Z_mean, X_var = Z_var, Xnew_mean_function = X_mean_function, 
            l = l, dim_layer = self.dim_layers[l], num_layers = self.num_layers, q_mu = q_mu, q_sqrt = q_sqrt, 
            log_lengthscales = log_lengthscales, log_kernel_variance = log_kernel_variance,               
            training_time = training_time, white = True, full_cov = full_cov)
